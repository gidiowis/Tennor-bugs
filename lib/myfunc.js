...
//want more free bot scripts? subscribe to my brother's youtube channel: https://youtube.com/@techgod143
...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = TennorBotInc.decodeJid(m.fromMe && XeonBotInc.user.id || m.participant || m.key.participant || m.chat || '')
        if (m.isGroup) m.participant = TennorBotInc.decodeJid(m.key.participant) || ''
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.sender = TennorBotInc.decodeJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === (TennorBotInc.user && XeonBotInc.user.id)
    ...
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? XeonBotInc.sendMedia(chatId, text, 'file', '', m, {
        ...options
    }) : TennorBotInc.sendText(chatId, text, m, {
        ...options
    })
    /**
     * Copy this message
